         ...
         <div class="border p-4 rounded">
            ...
            <form action="{{ route('login.authenticate') }}" method="POST" id="loginForm">
               ...
               <div class="d-flex justify-content-end">
                  <div class="mb-3">
                     <a href="{{ route('password.request') }}" class="link-primary link-offset-2 link-underline-opacity-0 link-underline-opacity-100-hover">Quên Mật Khẩu</a>
                  </div>
               </div>
               <button type="submit" class="btn btn-dark col-12 mt-4" id="loginBtn">Đăng Nhập</button>
            </form>
            <h6 class="text-center text-secondary my-2">Bạn chưa có tài khoản? <a href="/register" class="link-primary link-offset-2 link-underline-opacity-25 link-underline-opacity-100-hover">Đăng ký</a></h6>
         </div>
      </div>
   </div>
@endsection
            ...
            $.ajax({
               ...
               success: (response) => {
                  if (response.status === 'loginSuccess') {
                     ...
                     $('#loginFailedAlertMessage').text('Đăng nhập thất bại! Vui lòng kiểm tra đầu vào của bạn.');
                  }
               },
